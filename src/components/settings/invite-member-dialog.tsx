...
export function InviteMemberDialog({
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setResult({
        ...
        accountName: account?.name ?? 'our Brand Reach Solutions account',
      });
      onCreated();
    } catch (err) {
  ...
  function whatsappShareUrl(url: string): string {
    ...
    // for users in multi-team contexts where "our Brand Reach Solutions account"
    // wouldn't be enough to disambiguate.
    const accountName = result?.accountName ?? 'our Brand Reach Solutions account';
    const message = `Join ${accountName} on Brand Reach Solutions using this link (valid for ${result?.expiresInDays} days): ${url}`;
